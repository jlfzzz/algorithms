...
namespace detail {
    ...
    struct OutputHelper {
        ...
        const char *operator()(std::uint32_t value) const { return reinterpret_cast<const char *>(table + value); }
    };
    ...
    struct TransformHelper {
        ...
        void resize(std::uint32_t transformLength) {
            if (transformLength > length << 1) {
                ...
                for (std::uint32_t i = 0, j = (halfSize * 3) >> 1, phaseAccumulator = 0; i != halfSize;
                    ...
                    std::complex<double> first = std::polar(1.0, phaseAccumulator * angleStep),
                                         second = std::polar(1.0, phaseAccumulator * fineAngleStep);
                    baseFactors[i] = _mm_set_pd(first.imag(), first.real()),
                    baseFactors[i | halfSize] = _mm_set_pd(second.imag(), second.real());
                }
                __m128d *newFactors = reinterpret_cast<__m128d *>(
                    std::memcpy(new __m128d[transformLength >> 1], twiddleFactors, length << 4));
                delete[] twiddleFactors, twiddleFactors = newFactors;
                for (std::uint32_t i = length; i != transformLength >> 1; ++i)
                    twiddleFactors[i] =
                        complexMultiply(baseFactors[i & (halfSize - 1)], baseFactors[halfSize | (i >> halfLog)]);
        ...
        void decimationInFrequency(__m128d *dataArray, std::uint32_t transformSize) {
            for (std::uint32_t blockSize = transformSize >> 1, stepSize = transformSize; blockSize;
                 stepSize = blockSize, blockSize >>= 1) {
                ...
                for (__m128d *blockStart = dataArray + stepSize, *twiddlePointer = twiddleFactors + 1;
                     blockStart != dataArray + transformSize; blockStart += stepSize, ++twiddlePointer) {
                    for (__m128d *currentElement = blockStart; currentElement != blockStart + blockSize;
                         ++currentElement) {
                        const __m128d evenElement = *currentElement,
                                      oddElement = complexMultiply(currentElement[blockSize], *twiddlePointer);
            ...
            for (std::uint32_t blockSize = 1, stepSize = 2; blockSize < transformSize;
                ...
                for (__m128d *blockStart = dataArray + stepSize, *twiddlePointer = twiddleFactors + 1;
                    ...
                    for (__m128d *currentElement = blockStart; currentElement != blockStart + blockSize;
                        ...
                        const __m128d evenElement = *currentElement, oddElement = currentElement[blockSize];
                        *currentElement = evenElement + oddElement,
                        currentElement[blockSize] = complexMultiplyConjugate(evenElement - oddElement, *twiddlePointer);
                    }
                }
            }
        }
        ...
        void frequencyDomainPointwiseMultiply(__m128d *firstArray, __m128d *secondArray, std::uint32_t transformSize) {
            ...
            firstArray[0] =
                complexScalarMultiply(complexMultiplySpecial(firstArray[0], secondArray[0]), normalizationFactor);
            ...
            const __m128d negateMask =
                _mm_castsi128_pd(_mm_set_epi64x(std::int64_t(1ull << 63), std::int64_t(1ull << 63)));
            for (std::uint32_t blockStart = 2, blockEnd = 3; blockStart < transformSize;
                 blockStart <<= 1, blockEnd <<= 1) {
                for (std::uint32_t forwardIndex = blockStart, backwardIndex = forwardIndex + blockStart - 1;
                     forwardIndex < blockEnd; ++forwardIndex, --backwardIndex) {
                    const __m128d firstEven = _mm_add_pd(firstArray[forwardIndex],
                                                         _mm_xor_pd(firstArray[backwardIndex], conjugateMask)),
                                  firstOdd = _mm_sub_pd(firstArray[forwardIndex],
                                                        _mm_xor_pd(firstArray[backwardIndex], conjugateMask));
                    const __m128d secondEven = _mm_add_pd(secondArray[forwardIndex],
                                                          _mm_xor_pd(secondArray[backwardIndex], conjugateMask)),
                                  secondOdd = _mm_sub_pd(secondArray[forwardIndex],
                                                         _mm_xor_pd(secondArray[backwardIndex], conjugateMask));
                    const __m128d productA = _mm_sub_pd(
                                      complexMultiply(firstEven, secondEven),
                                      complexMultiply(complexMultiply(firstOdd, secondOdd),
                                                      (forwardIndex & 1
                                                           ? _mm_xor_pd(twiddleFactors[forwardIndex >> 1], negateMask)
                                                           : twiddleFactors[forwardIndex >> 1]))),
                                  productB = _mm_add_pd(complexMultiply(secondEven, firstOdd),
                                                        complexMultiply(firstEven, secondOdd));
                    firstArray[forwardIndex] = complexScalarMultiply(_mm_add_pd(productA, productB), scalingFactor);
                    firstArray[backwardIndex] =
                        _mm_xor_pd(complexScalarMultiply(_mm_sub_pd(productA, productB), scalingFactor), conjugateMask);
                }
            }
        }
    };
...
protected:
    UnsignedInteger(std::uint32_t initialLength, std::uint32_t initialCapacity) :
        digits(new std::uint32_t[initialCapacity]), length(initialLength), capacity(initialCapacity) {}

    void resize(std::uint32_t newLength) {
        if (newLength > capacity) {
            std::uint32_t *newMemory = reinterpret_cast<std::uint32_t *>(
                std::memcpy(new std::uint32_t[capacity = newLength], digits, length << 2));
        ...
        switch (stringLength & 7) {
            ...
            case 6:
                *currentDigit = detail::I(value) * 10000 + detail::I(value + 2) * 100 + detail::I(value + 4);
                break;
            case 7:
                *currentDigit = (*value & 15) * 1000000 + detail::I(value + 1) * 10000 + detail::I(value + 3) * 100 +
                                detail::I(value + 5);
                break;
        }
        for (const char *position = value + (stringLength & 7); currentDigit != digits;
             *--currentDigit = detail::I(position) * 1000000 + detail::I(position + 2) * 10000 +
                               detail::I(position + 4) * 100 + detail::I(position + 6),
                        position += 8)
            ;
    ...
    std::int32_t reverseCompare(const UnsignedInteger &other) const {
        ...
        const std::uint32_t remaining = length % BlockSize, *firstBlockPointer = digits + length - remaining,
                            *secondBlockPointer = other.digits + length - remaining;
        if (std::memcmp(firstBlockPointer, secondBlockPointer, remaining << 2))
            for (const std::uint32_t *firstElementPointer = firstBlockPointer + remaining,
                                     *secondElementPointer = secondBlockPointer + remaining;
                 firstElementPointer != firstBlockPointer;)
        ...
        while (firstBlockPointer != digits)
            if (std::memcmp(firstBlockPointer -= BlockSize, secondBlockPointer -= BlockSize, BlockSize << 2))
                for (const std::uint32_t *firstElementPointer = firstBlockPointer + BlockSize,
                                         *secondElementPointer = secondBlockPointer + BlockSize;
                     firstElementPointer != firstBlockPointer;)
                    if (*--firstElementPointer != *--secondElementPointer)
                        return std::int32_t(*firstElementPointer - *secondElementPointer);
        return 0;
    ...
    std::pair<UnsignedInteger, UnsignedInteger> bruteforceDivisionAndModulus(const UnsignedInteger &divisor) const {
        ...
        auto getEstimatedValue = [](const std::uint32_t *digitArray, std::uint32_t highIndex,
                                    std::uint32_t arrayLength) -> std::uint64_t {
            return std::uint64_t(10) * Base * ((highIndex + 1) < arrayLength ? digitArray[highIndex + 1] : 0) +
                   std::uint64_t(10) * digitArray[highIndex] +
                   (highIndex ? digitArray[highIndex - 1] : 0) / (Base / 10);
        };
        for (std::uint32_t currentPosition = length - divisor.length; ~currentPosition; --currentPosition) {
            ...
            auto performSubtraction = [&]() {
                ...
                for (std::uint32_t digitIndex = 0; digitIndex != divisor.length; ++digitIndex) {
                    carry = carry - std::int64_t(partialQuotient) * divisor.digits[digitIndex] +
                            remainder.digits[currentPosition + digitIndex];
            ...
            for (quotient.digits[currentPosition] = 0;
                 (partialQuotient =
                      std::uint32_t(getEstimatedValue(remainder.digits, currentPosition + divisor.length - 1, length) /
                                    (getEstimatedValue(divisor.digits, divisor.length - 1, divisor.length) + 1)));
                 performSubtraction())
                ;
            partialQuotient = 1;
            for (std::uint32_t digitIndex = divisor.length; digitIndex--;)
                if (remainder.digits[digitIndex + currentPosition] != divisor.digits[digitIndex] &&
                    (partialQuotient = divisor.digits[digitIndex] < remainder.digits[digitIndex + currentPosition],
                     true))
                    break;
            if (partialQuotient)
                performSubtraction();
    ...
    UnsignedInteger computeInverse(std::uint32_t precisionBits) const {
        ...
        const std::uint32_t halfPrecision = (precisionBits - length + 5) >> 1,
                            shiftBack = halfPrecision > length ? 0 : length - halfPrecision;
        ...
        UnsignedInteger result =
            (approximateInverse + approximateInverse).leftShift(precisionBits - newPrecision - shiftBack) -
            (*this * approximateInverse * approximateInverse)
                .rightShift(2 * (newPrecision + shiftBack) - precisionBits);
        return --result;
    }
    ...
    std::pair<UnsignedInteger, UnsignedInteger> divisionAndModulus(const UnsignedInteger &other) const {
        ...
        const std::uint32_t precisionBits = length - other.length + 5,
                            shiftBack = precisionBits > other.length ? 0 : other.length - precisionBits;
        ...
        UnsignedInteger quotient = (*this * adjustedDivisor.computeInverse(inversePrecision))
                                       .rightShift(inversePrecision + shiftBack),
...
public:
    ...
    UnsignedInteger(const UnsignedInteger &other) :
        digits(reinterpret_cast<std::uint32_t *>(
            std::memcpy(new std::uint32_t[other.length], other.digits, other.length << 2))),
        length(other.length), capacity(other.length) {}

    UnsignedInteger(UnsignedInteger &&other) noexcept :
        digits(other.digits), length(other.length), capacity(other.capacity) {
    ...
    UnsignedInteger(const SignedInteger &other) : UnsignedInteger(*reinterpret_cast<const UnsignedInteger *>(&other)) {
        VALIDITY_CHECK(!(reinterpret_cast<const std::pair<UnsignedInteger, bool> *>(&other)->second),
                       std::invalid_argument,
                       "UnsignedInteger constructor error: the provided SignedInteger is negative. UnsignedInteger can "
                       "only represent non-negative integers.")
    }

    template<typename unsignedIntegral,
             typename std::enable_if<std::is_unsigned<unsignedIntegral>::value>::type * = nullptr>
    UnsignedInteger(unsignedIntegral value) :
        UnsignedInteger(0, (std::numeric_limits<unsignedIntegral>::digits10 + 7) >> 3) {
        while (digits[length++] = std::uint32_t(value) % Base, value /= unsignedIntegral(Base))
            ;
    }

    template<typename signedIntegral,
             typename std::enable_if<std::is_signed<signedIntegral>::value &&
                                     !std::is_floating_point<signedIntegral>::value>::type * = nullptr>
    UnsignedInteger(signedIntegral value) :
        UnsignedInteger(0, (std::numeric_limits<signedIntegral>::digits10 + 7) >> 3) {
        VALIDITY_CHECK(
            value >= 0, std::invalid_argument,
            "UnsignedInteger constructor error: the provided signed integer value = " + std::to_string(value) +
    ...
    UnsignedInteger(floatingPoint value) :
        UnsignedInteger(0, (std::numeric_limits<floatingPoint>::max_exponent10 + 7) >> 3) {
        VALIDITY_CHECK(
            value >= 0, std::invalid_argument,
            "UnsignedInteger constructor error: the provided floating point value = " + std::to_string(value) +
                " is negative. UnsignedInteger can only represent non-negative integers.")
        VALIDITY_CHECK(std::isfinite(value), std::invalid_argument,
    ...
    UnsignedInteger(const char *value) {
        VALIDITY_CHECK(value, std::invalid_argument,
        ...
        VALIDITY_CHECK(stringLength, std::invalid_argument,
                       "UnsignedInteger constructor error: the provided C-style string is empty. UnsignedInteger can "
                       "only be constructed from non-empty strings containing only digits.")
        VALIDITY_CHECK(std::all_of(value, value + stringLength, [](char digit) -> bool { return std::isdigit(digit); }),
                       std::invalid_argument,
                       "UnsignedInteger constructor error: the provided C-style string value = \"" +
                           std::string(value) +
                           "\" contains non-digit characters. UnsignedInteger can only be constructed from non-empty "
                           "strings containing only digits.")
        construct(value, stringLength);
    }

    UnsignedInteger(const std::string &value) :
        UnsignedInteger(std::uint32_t(value.size() + 7) >> 3, std::uint32_t(value.size() + 7) >> 3) {
        VALIDITY_CHECK(value.size(), std::invalid_argument,
                       "UnsignedInteger constructor error: the provided string is empty. UnsignedInteger can only be "
                       "constructed from non-empty strings containing only digits.")
        VALIDITY_CHECK(std::all_of(value.begin(), value.end(), [](char digit) -> bool { return std::isdigit(digit); }),
                       std::invalid_argument,
                       "UnsignedInteger constructor error: the provided string value = \"" + value +
                           "\" contains non-digit characters. UnsignedInteger can only be constructed from non-empty "
                           "strings containing only digits.")
        construct(value.data(), std::uint32_t(value.size()));
    }

    ~UnsignedInteger() noexcept { delete[] digits; }
    ...
    UnsignedInteger &operator=(const SignedInteger &other) {
        VALIDITY_CHECK(!(reinterpret_cast<const std::pair<UnsignedInteger, bool> *>(&other)->second),
                       std::invalid_argument,
                       "UnsignedInteger operator= error: the provided SignedInteger is negative. UnsignedInteger can "
                       "only represent non-negative integers.")
    ...
    UnsignedInteger &operator=(unsignedIntegral value) {
        if (length = 0, capacity < (std::numeric_limits<unsignedIntegral>::digits10 + 7) >> 3)
            delete[] digits,
                digits = new std::uint32_t[capacity = (std::numeric_limits<unsignedIntegral>::digits10 + 7) >> 3];
        while (digits[length++] = std::uint32_t(value) % Base, value /= unsignedIntegral(Base))
            ;
        return *this;
    }
    ...
    UnsignedInteger &operator=(signedIntegral value) {
        VALIDITY_CHECK(value >= 0, std::invalid_argument,
                       "UnsignedInteger operator= error: the provided signed integer value = " + std::to_string(value) +
                           " is negative. UnsignedInteger can only represent non-negative integers.")
        if (length = 0, capacity < (std::numeric_limits<signedIntegral>::digits10 + 7) >> 3)
            delete[] digits,
                digits = new std::uint32_t[capacity = (std::numeric_limits<signedIntegral>::digits10 + 7) >> 3];
        while (digits[length++] = std::uint32_t(value) % Base, value /= signedIntegral(Base))
            ;
        return *this;
    }
    ...
    UnsignedInteger &operator=(floatingPoint value) {
        VALIDITY_CHECK(value >= 0, std::invalid_argument,
                       "UnsignedInteger operator= error: the provided floating point value = " + std::to_string(value) +
                           " is negative. UnsignedInteger can only represent non-negative integers.")
        VALIDITY_CHECK(std::isfinite(value), std::invalid_argument,
                       "UnsignedInteger operator= error: the provided floating point value = " + std::to_string(value) +
                           " is not finite.")
        if (length = 0, capacity < (std::numeric_limits<floatingPoint>::max_exponent10 + 7) >> 3)
            delete[] digits,
                digits = new std::uint32_t[capacity = (std::numeric_limits<floatingPoint>::max_exponent10 + 7) >> 3];
        while (digits[length++] = std::uint32_t(std::fmod(value, Base)), (value = std::floor(value / Base)))
            ;
        return *this;
    }
    ...
    UnsignedInteger &operator=(const char *value) {
        ...
        const std::uint32_t stringLength = std::uint32_t(std::strlen(value));
        VALIDITY_CHECK(stringLength, std::invalid_argument,
                       "UnsignedInteger operator= error: the provided C-style string is empty. UnsignedInteger can "
                       "only be constructed from non-empty strings containing only digits.")
        VALIDITY_CHECK(std::all_of(value, value + stringLength, [](char digit) -> bool { return std::isdigit(digit); }),
                       std::invalid_argument,
                       "UnsignedInteger operator= error: the provided C-style string value = \"" + std::string(value) +
                           "\" contains non-digit characters. UnsignedInteger can only be constructed from non-empty "
                           "strings containing only digits.")
    ...
    UnsignedInteger &operator=(const std::string &value) {
        VALIDITY_CHECK(value.size(), std::invalid_argument,
                       "UnsignedInteger operator= error: the provided string is empty. UnsignedInteger can only be "
                       "constructed from non-empty strings containing only digits.")
        VALIDITY_CHECK(std::all_of(value.begin(), value.end(), [](char digit) -> bool { return std::isdigit(digit); }),
                       std::invalid_argument,
                       "UnsignedInteger operator= error: the provided string value = \"" + value +
                           "\" contains non-digit characters. UnsignedInteger can only be constructed from non-empty "
                           "strings containing only digits.")
    ...
    operator unsignedIntegral() const {
        ...
        for (std::uint32_t *i = digits + length; i != digits;
             result = result * unsignedIntegral(Base) + unsignedIntegral(*--i))
            ;
        return result;
    }
    ...
    operator signedIntegral() const {
        ...
        for (std::uint32_t *i = digits + length; i != digits;
             result = result * signedIntegral(Base) + signedIntegral(*--i))
            ;
        return result;
    }
    ...
    operator const char *() const {
        ...
        for (std::uint32_t number = *--i; *--resultPointer = char(48 | number % 10), ++numberLength, number /= 10;)
            ;
        for (std::memmove(result, resultPointer, numberLength), resultPointer = result + numberLength; i-- != digits;
             std::memcpy(resultPointer, detail::O(*i / 10000), 4), resultPointer += 4,
                                                                std::memcpy(resultPointer, detail::O(*i % 10000), 4),
                                                                resultPointer += 4)
            ;
    ...
    operator std::string() const {
        ...
        for (result.reserve(length << 3); i-- != digits;
             result.append(detail::O(*i / 10000), 4), result.append(detail::O(*i % 10000), 4))
            ;
        return result;
    }
    ...
    UnsignedInteger &operator+=(const UnsignedInteger &other) {
        ...
        for (std::uint32_t *otherDigit = other.digits, *otherEnd = other.digits + other.length; otherDigit != otherEnd;
    ...
    UnsignedInteger &operator-=(const UnsignedInteger &other) {
        VALIDITY_CHECK(*this >= other, std::invalid_argument,
                       "UnsignedInteger subtraction error: Attempted to subtract a larger UnsignedInteger \"" +
                           other.operator std::string() + "\" from a smaller one \"" + operator std::string() + "\".")
    ...
    UnsignedInteger &operator--() {
        VALIDITY_CHECK(bool(*this), std::invalid_argument,
                       "UnsignedInteger subtraction error: Attempted to subtract a larger UnsignedInteger \"1\" from a "
                       "smaller one \"0\".")
        std::uint32_t *thisDigit = digits, *thisEnd = digits + length - 1;
        for (--*thisDigit; thisDigit != thisEnd && *thisDigit >= Base; *thisDigit += Base, --*++thisDigit)
            ;
    ...
    UnsignedInteger &operator*=(const UnsignedInteger &other) {
        ...
        VALIDITY_CHECK(length <= TransformLimit, std::invalid_argument,
                       "UnsignedInteger multiplication error: left operand length (" + std::to_string(length) +
                           ") exceeds Transform limit (" + std::to_string(TransformLimit) + ").")
        VALIDITY_CHECK(other.length <= TransformLimit, std::invalid_argument,
                       "UnsignedInteger multiplication error: right operand length (" + std::to_string(other.length) +
                           ") exceeds Transform limit (" + std::to_string(TransformLimit) + ").")
        ...
        if (allocatedSize < transformLength << 1)
            delete[] firstArray, delete[] secondArray, firstArray = new double[transformLength << 1](),
                                                       secondArray = new double[transformLength << 1](),
                                                       allocatedSize = transformLength << 1;
        std::memset(firstArray, 0, transformLength << 4), std::memset(secondArray, 0, transformLength << 4);
        for (std::uint32_t i = 0; i != length;
             firstArray[i << 1] = digits[i] % 10000u, firstArray[i << 1 | 1] = digits[i] / 10000u, ++i)
            ;
        for (std::uint32_t i = 0; i != other.length;
             secondArray[i << 1] = other.digits[i] % 10000u, secondArray[i << 1 | 1] = other.digits[i] / 10000u, ++i)
            ;
        detail::T.resize(transformLength),
            detail::T.decimationInFrequency(reinterpret_cast<__m128d *>(firstArray), transformLength),
            detail::T.decimationInFrequency(reinterpret_cast<__m128d *>(secondArray), transformLength);
        detail::T.frequencyDomainPointwiseMultiply(reinterpret_cast<__m128d *>(firstArray),
                                                   reinterpret_cast<__m128d *>(secondArray), transformLength),
            detail::T.decimationInTime(reinterpret_cast<__m128d *>(firstArray), transformLength);
        UnsignedInteger result(resultLength, resultLength);
        std::uint64_t carry = 0;
        for (std::uint32_t i = 0; i < resultLength; ++i) {
            carry += std::uint64_t(std::int64_t(firstArray[i << 1] + 0.5) +
                                   std::int64_t(firstArray[i << 1 | 1] + 0.5) * 10000);
            result.digits[i] = std::uint32_t(carry % Base), carry /= Base;
        }
        for (; carry && result.length < result.capacity;
             result.digits[result.length++] = std::uint32_t(carry % Base), carry /= Base)
...
UnsignedInteger operator""_UI(const char *literal, std::size_t) { return UnsignedInteger(literal); }
...
protected:
    SignedInteger(const UnsignedInteger &initialAbsolute, bool initialSign) :
        absolute(initialAbsolute), sign(initialSign) {}
...
public:
    ...
    SignedInteger(const char *value) {
        ...
        VALIDITY_CHECK(*value, std::invalid_argument,
                       "SignedInteger constructor error: the provided C-style string is empty. SignedInteger can only "
                       "be constructed from non-empty strings containing only digits (optionally prefixed with '-').")
        VALIDITY_CHECK(std::all_of(value + (*value == '-'), value + std::strlen(value),
                                   [](char digit) -> bool { return std::isdigit(digit); }),
                       std::invalid_argument,
                       "SignedInteger constructor error: the provided C-style string value = \"" + std::string(value) +
                           "\" contains non-digit characters. SignedInteger can only be constructed from non-empty "
                           "strings containing only digits (optionally prefixed with '-').")
        absolute = value + (sign = *value == '-'), sign = sign && bool(absolute);
    }

    SignedInteger(const std::string &value) {
        VALIDITY_CHECK(value.size(), std::invalid_argument,
                       "SignedInteger constructor error: the provided string is empty. SignedInteger can only be "
                       "constructed from non-empty strings containing only digits (optionally prefixed with '-').")
        VALIDITY_CHECK(std::all_of(value.begin() + (value.front() == '-'), value.end(),
                                   [](char digit) -> bool { return std::isdigit(digit); }),
                       std::invalid_argument,
                       "SignedInteger constructor error: the provided string value = \"" + value +
                           "\" contains non-digit characters. SignedInteger can only be constructed from non-empty "
                           "strings containing only digits (optionally prefixed with '-').")
    ...
    SignedInteger &operator=(const SignedInteger &other) { return absolute = other.absolute, sign = other.sign, *this; }

    SignedInteger &operator=(SignedInteger &&other) noexcept {
        return absolute = std::move(other.absolute), sign = other.sign, *this;
    }

    SignedInteger &operator=(const UnsignedInteger &other) { return absolute = other, sign = false, *this; }
    ...
    SignedInteger &operator=(const char *value) {
        ...
        VALIDITY_CHECK(*value, std::invalid_argument,
                       "SignedInteger operator= error: the provided C-style string is empty. SignedInteger can only be "
                       "assigned from non-empty strings containing only digits (optionally prefixed with '-').")
        VALIDITY_CHECK(std::all_of(value + (*value == '-'), value + std::strlen(value),
                                   [](char digit) -> bool { return std::isdigit(digit); }),
                       std::invalid_argument,
                       "SignedInteger operator= error: the provided C-style string value = \"" + std::string(value) +
                           "\" contains non-digit characters. SignedInteger can only be assigned from non-empty "
                           "strings containing only digits (optionally prefixed with '-').")
    ...
    SignedInteger &operator=(const std::string &value) {
        VALIDITY_CHECK(value.size(), std::invalid_argument,
                       "SignedInteger operator= error: the provided string is empty. SignedInteger can only be "
                       "assigned from non-empty strings containing only digits (optionally prefixed with '-').")
        VALIDITY_CHECK(std::all_of(value.begin() + (value.front() == '-'), value.end(),
                                   [](char digit) -> bool { return std::isdigit(digit); }),
                       std::invalid_argument,
                       "SignedInteger operator= error: the provided string value = \"" + value +
                           "\" contains non-digit characters. SignedInteger can only be assigned from non-empty "
                           "strings containing only digits (optionally prefixed with '-').")
    ...
    template<typename unsignedIntegral,
             typename std::enable_if<std::is_unsigned<unsignedIntegral>::value>::type * = nullptr>
    operator unsignedIntegral() const {
        VALIDITY_CHECK(!sign, std::invalid_argument,
    ...
    operator bool() const noexcept { return bool(absolute); }

#if __cplusplus >= 202002L
    std::strong_ordering operator<=>(const SignedInteger &other) const {
        return sign != other.sign ? other.sign <=> sign
                                  : (sign ? other.absolute <=> absolute : absolute <=> other.absolute);
    }
#endif

    bool operator==(const SignedInteger &other) const { return sign == other.sign && absolute == other.absolute; }
    ...
    SignedInteger &operator+=(const SignedInteger &other) {
        sign == other.sign ? absolute += other.absolute
                           : (absolute < other.absolute            ? sign = !sign,
                              absolute = other.absolute - absolute : absolute -= other.absolute);
...
SignedInteger operator""_SI(const char *literal, std::size_t) { return SignedInteger(literal); }
