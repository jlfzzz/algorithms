...
void solve() {
    ...
    D(i, n - 1, 0) {
        F(bal, 0, n) {
            F(len, 0, n - i) {
                dp[i][bal][len] = dp[i + 1][bal][len];

                if (len > 0) {
                    if (s[i] == '(') {
                        if (bal + 1 <= n) {
                            dp[i][bal][len] += dp[i + 1][bal + 1][len - 1];
                        }
                    } else {
                        if (bal - 1 >= 0) {
                            dp[i][bal][len] += dp[i + 1][bal - 1][len - 1];
    ...
    F(j, 0, n - 1) {
        if (s[j] == ')') {
            F(k, 1, pre) {
                Z term1 = comb.C(pre, k);

                int needed = k - 1;

                F(suf, 0, n - 1 - j) {
                    int pairs = suf - needed;
                    int r1 = pairs / 2;
                    ...
                    if (l1 >= 1 && R >= k + 1) {
                        int len = k + 1 + suf;
                        ans += term1 * dp[j + 1][needed][suf] * (len - 2);
                    }
                }
            }
        } else {
