...
Z fib(long long n, int an, int an_1) {
    if (n == 0)
        return Z(an);
...
void solve() {
    ...
    auto mat0 = Matrix(1, vector<Z>(2));
    mat0[0][0] = s1;
    mat0[0][1] = 1;

    auto unit = Matrix(2, vector<Z>(2));
    unit[0][0] = 3;
    unit[0][1] = 0;
    unit[1][0] = -(a1 + an);
    unit[1][1] = 1;

    auto mat1 = quick_mul(unit, x);
    mat1 = mat_mul(mat0, mat1);

    Z mx = fib(x, an, a[n - 2]);

    unit[1][0] = -(Z(a1) + mx);

    auto mat2 = quick_mul(unit, y);
    auto mat3 = mat_mul(mat1, mat2);
