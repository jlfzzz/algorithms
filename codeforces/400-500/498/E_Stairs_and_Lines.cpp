...
constexpr int STATES = 1 << ROWS;
constexpr int FULL_MASK = STATES - 1;

vector<Matrix> trans(8);
bool built = false;

bool mask_valid(int mask, int h) {
    int forced = FULL_MASK ^ ((1 << h) - 1);
    return (mask & forced) == forced;
}

Matrix build_matrix(int h) {
    Matrix ret(STATES, vector<Z>(STATES, 0));
    vector<int> states;
    states.reserve(1 << h);
    for (int mask = 0; mask < STATES; mask++) {
        if (mask_valid(mask, h)) {
            states.pb(mask);
        }
    }
    int total = 1 << (h - 1);
    for (int left_mask: states) {
        for (int right_mask: states) {
            Z ways = 0;
...
vector<Z> vec_mul(const vector<Z> &vec, const Matrix &mat) {
    int n = vec.size();
    int m = mat[0].size();
    vector<Z> res(m, 0);
    for (int i = 0; i < n; i++) {
...
vector<Z> apply_pow(vector<Z> vec, Matrix mat, long long times) {
    if (times == 0) {
        return vec;
    }
    long long exp = times;
...
void solve() {
    ...
    vector<Z> dp(STATES, 0);
    dp[FULL_MASK] = 1;
    for (int h = 1; h <= ROWS; h++) {
        if (w[h] == 0) {
            continue;
        }
        dp = apply_pow(dp, trans[h], w[h]);
    }
    prt(dp[FULL_MASK].val());
}
