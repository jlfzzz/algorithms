...
void solve() {
    ...
    vector<S> tree_arr0(ts + 2), tree_arr1(ts + 2);
    L(i, 1, n) {
        if (depth[i] % 2 == 0) {
            tree_arr0[in[i]] = {a[i]};
            tree_arr1[in[i]] = {0};
    ...
    atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg0(tree_arr0), seg1(tree_arr1);
    ...
    while (m--) {
        ...
        if (type == 1) {
            ...
            if (depth[x] % 2 == 0) {
                seg0.apply(in[x], out[x] + 1, F{+val});
                seg1.apply(in[x], out[x] + 1, F{-val});
            } else {
                seg0.apply(in[x], out[x] + 1, F{-val});
                seg1.apply(in[x], out[x] + 1, F{+val});
            }
            ...
            rd(x);
            if (depth[x] % 2 == 0) {
                prt(seg0.get(in[x]).val);
            } else {
                prt(seg1.get(in[x]).val);
            }
        }
    }
}
