...
constexpr int MOD1 = 95542721;
constexpr int LEN = 48;
...
S e() {
    S res;
    memset(res.sums, 0, sizeof(res.sums));
    return res;
}
...
S mapping(F f, S x) {
    ...
    int s = f.lazy;
    for (int i = 0; i < LEN; i++) {
        res.sums[i] = x.sums[(i + s) % LEN];
    }
    return res;
}

F composition(F f, F g) { return {(f.lazy + g.lazy) % LEN}; }
