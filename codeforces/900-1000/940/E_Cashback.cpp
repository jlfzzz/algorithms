...
void solve() {
    ...
    vector<S> tree_arr1(n + 1), tree_arr2(n + 1);
    L(i, 1, n) {
        tree_arr1[i] = {a[i], a[i]};
        tree_arr2[i] = {INF, -INF};
    }
    ...
    atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg(tree_arr1), dp(tree_arr2);
    L(i, c, n) {
        auto mx = dp.prod(0, i).mx;
        if (i - c + 1 >= 1) {
            mx = max(mx, seg.prod(i - c + 1, i + 1).mn + dp.prod(0, i - c + 1).mx);
        }
        dp.set(i, {mx, mx});
    }

    ll sum = accumulate(all2(a, 1), 0ll);
    sum -= dp.prod(0, n + 1).mx;
