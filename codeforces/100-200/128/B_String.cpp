...
struct SuffixAutomaton {
    ...
    vector<long long> dp_dup;

    // 计算不重复子串的路径数 (每个节点贡献 1)
    void calc_distinct_paths(int u) {
        if (dp_distinct[u] != -1)
            return;
        dp_distinct[u] = 1;
        for (int c = 0; c < SIGMA; c++) {
            if (st[u].next[c] != -1) {
                calc_distinct_paths(st[u].next[c]);
                dp_distinct[u] += dp_distinct[st[u].next[c]];
            }
        }
    }

    // 计算重复子串的加权路径数 (每个节点贡献 occ[u])
    void calc_dup_paths(int u) {
        if (dp_dup[u] != -1)
            return;
        dp_dup[u] = occ[u];
        for (int c = 0; c < SIGMA; c++) {
            if (st[u].next[c] != -1) {
                calc_dup_paths(st[u].next[c]);
                dp_dup[u] += dp_dup[st[u].next[c]];
            }
        }
    }

    // 获取第 K 小的子串 (不重复计算，本质是 DAG 上的第 K 条路径)
    string get_kth_distinct(long long k) {
        dp_distinct.assign(st.size(), -1);
        calc_distinct_paths(0);
    ...
    // 获取第 K 小的子串 (计算重复出现次数)
    string get_kth_duplicated(long long k) {
        assert(occ.size() == st.size() && "Must call prepare_occurrences() before querying duplicated kth!");

        dp_dup.assign(st.size(), -1);

        long long old_root_occ = occ[0];
        occ[0] = 0;
...
void solve() {
    ...
    cout << sam.get_kth_duplicated(k) << endl;
}
...
void solve2() {
    ...
    string ans;
    auto dfs = [&](this auto &&dfs, vi &a, int kk) -> void {
        dbg(a, kk);
        if (kk == 0 || a.empty()) {
            return;
        }
