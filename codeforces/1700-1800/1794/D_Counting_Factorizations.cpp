...
struct Sieve {
    bool is_not_prime[MAXA + 1]{};
    std::vector<int> primes;
    int min_prime_factor[MAXA + 1]{};
    int distinct_factors_count[MAXA + 1]{};
...
void solve() {
    ...
    for (auto &[val, c]: cnt) {
        vector<Z> ndp(n + 1);
        for (int j: range(n + 1)) {
            ndp[j] = dp[j] * comb.C(n - (sum - j), c);
            if (j - 1 >= 0 && sieve.is_prime(val)) {
                ndp[j] += dp[j - 1] * comb.C(n - (sum - j + 1), c - 1);
            }
        }
        sum += c;
        dp.swap(ndp);
