...
void solve() {
    ...
    if (x > 0) {
        ...
        Matrix base(x + 1, vector<Z>(1, 0));
        F(j, 0, x - 1) { base[j][0] = 1; }
        base[x][0] = x;
        Matrix mat1 = quick_mul(mat, n - 1);
        Matrix res = mat_mul(mat1, base);
        bad = res[x][0];
    }

    Z total = q_pow(2 * k + 1, n - 1) * (Z) (x + k);
    total -= bad;
    prt(total.val());
}
