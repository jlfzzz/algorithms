...
void solve() {
    ...
    auto check = [&](int x, int y, bool f) -> bool {
        map<pii, int, greater<>> m1, m2;
        for (auto [x, y]: a)
            m1[make_pair(x, y)]++, m2[make_pair(y, x)]++;
        for (int c = 0; c < a.size(); f ^= 1) {
            if (f) {
                if (m1.begin()->first.first != x)
                    return false;
    ...
    if (!(c % mx) && check(mx, c / mx, 1)) {
        ans.eb(mx, c / mx);
    }
    if (mx != c / my && !(c % my) && check(c / my, my, 0))
        ans.emplace_back(c / my, my);
