...
void solve() {
    ...
    vector<atcoder::lazy_segtree<S, op, e, F, mapping, composition, id>> dp(k + 1);
    L(j, 0, k) {
        dp[j] = atcoder::lazy_segtree<S, op, e, F, mapping, composition, id>(n + 5);
        dp[j].set(0, {0});
    }
    ...
    L(i, 1, n) {
        ...
        for (int j = cnt; j <= k; ++j) {
            ...
            for (int idx = 0; idx <= cnt; ++idx) {
                ...
                int L = Ls[idx];
                int R = (idx == cnt ? i - 1 : Ls[idx + 1] - 1);
                if (L > R)
                    continue;

                int prev = dp[pj].prod(L, R + 1).mx;
                if (prev <= -inf)
                    continue;
