...
void solve() {
    ...
    auto check = [&](int l, int r) {
        int tl = seg.prod(l, l + 1).sum % 26;
        int tr = seg.prod(r, r + 1).sum % 26;
        if (r == l + 1) {
            bad1.erase({l, r});
            if (tl == tr) {
                bad1.insert({l, r});
    ...
    for (int i = 1; i <= m; i++) {
        ...
        if (type == 1) {
            ...
            auto it1 = bad1.lower_bound({l, -1});
            if (it1 != bad1.end() && it1->first <= r && it1->second <= r) {
                // dbg(*it1);
                prt("NO");
                continue;
            }

            it1 = bad2.lower_bound({l, -1});
            if (it1 != bad2.end() && it1->first <= r && it1->second <= r) {
                // dbg(*it1);
                prt("NO");
                continue;
            }
