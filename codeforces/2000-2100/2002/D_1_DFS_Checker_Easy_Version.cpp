...
void solve() {
    ...
    auto check = [&](int u) -> bool {
        int mn_pos = inf, mx_pos = -inf;
        for (auto [L, R]: sub_ranges[u]) {
            auto cur = seg.prod(L, R);
            mn_pos = min(mn_pos, cur.mn);
            mx_pos = max(mx_pos, cur.mx);
        }
        if (mx_pos - mn_pos + 1 != sub_size[u])
            return false;
        if (pos[u] != mn_pos)
            return false;
        return true;
    ...
    while (q--) {
        ...
        pos[a] = y;
        pos[b] = x;
        seg.apply(a, a + 1, {pos[a]});
        seg.apply(b, b + 1, {pos[b]});
