...
void solve() {
    ...
    For(i, n) {
        ...
        int first = mex;
        mx_initial = max(mx_initial, first);
        mex++;
        while (st.contains(mex)) {
            ++mex;
        }
        graph[first].push_back(mex);
        mx_mex = max(mx_mex, mex);
    }

    vector<int> dp(mx_mex + 1);
    int mx = mx_initial;
    for (int i = mx_mex; i >= 0; i--) {
        dp[i] = i;
        for (int v: graph[i]) {
            dp[i] = max(dp[i], dp[v]);
        }
    ...
    ll ans = 0;
    for (int i = 0; i <= min(m, mx_mex); i++) {
        ans += max(mx, dp[i]);
    }
    if (m > mx_mex) {
        ans += 1ll * (m + mx_mex + 1) * (m - mx_mex) / 2;
    }
    cout << ans << '\n';
}
