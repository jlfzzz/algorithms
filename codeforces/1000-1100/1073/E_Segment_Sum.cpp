...
//         auto dfs = [&](this auto &&dfs, int i, int mask, bool lim, bool notNum) -> pair<Z, Z> {
//             if (popcount((unsigned) mask) > k) {
//                 return {0, 0};
//             }
...
//                 auto [cnt, sum] = dfs(i + 1, nMask, lim && (d == hi), notNum && (d == 0));

//                 res.first += cnt;
//                 Z val = (Z) d * pow10[len - 1 - i];
//                 res.second += sum + val * cnt;
//             }
...
void solve() {
    ...
    auto calc = [&](ll n) -> Z {
        ...
        auto dfs = [&](this auto &&dfs, int i, int mask, bool lim, bool notNum) -> pair<Z, Z> {
            ...
            if (vis[lim][notNum][i][mask]) {
                return dp[lim][notNum][i][mask];
            }
            ...
            vis[lim][notNum][i][mask] = 1;
            dp[lim][notNum][i][mask] = res;
