...
void solve() {
    ...
    vector<Z> pre(n + 1), pow10(n + 1);
    pow10[0] = 1;
    F(i, 1, n) {
        Z cur = s[i] - '0';
        pre[i] = pre[i - 1] * 10 + cur;
        pow10[i] = pow10[i - 1] * 10;
    }

    auto calc = [&](int L, int R) { return pre[R] - pre[L - 1] * pow10[R - L + 1]; };
    ...
    if (len <= n && idxL <= idxR) {
        ...
        ll mid = len - lenL - lenR;

        if (mid < 0) {
            ans = valL * pow10[lenR] + valR;
        } else {
            ll k = mid / n;
            ...
            sum = vall * (q_pow(q, k) - 1) * Z(q - 1).inv();

            Z t1 = q_pow(q, k);
            Z t2 = pow10[lenR];

            ans = valL * t1 * t2 + sum * t2 + valR;
        }
    }
