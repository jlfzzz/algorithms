...
void solve() {
    ...
    vector<vector<Z>> dp1(n + 1, vector<Z>(M)), dp2(n + 1, vector<Z>(M));
    dp1[0][0] = 1;
    dp2[0][0] = 1;
    ...
    for (int x: b) {
        ...
        for (int i = n - 1; i >= 0; i--) {
            for (int j = 0; j < M; j++) {
                dp2[i + 1][(j + m) % M] += dp2[i][j];
            }
        }
    }

    vector<vector<Z>> sum_dp2(n + 1, vector<Z>(M));
    for (int i = 0; i <= n; i++) {
        for (int j = 0; j < M; j++) {
            sum_dp2[i][j] = dp2[i][j];
            if (i)
                sum_dp2[i][j] += sum_dp2[i - 1][j];
        }
    }
