...
S op(S a, S b) { return {a.sum + b.sum, min(a.mn, b.mn), a.len + b.len}; }
...
S mapping(F f, S x) {
    if (f.lazy == 0)
        return x;
    return {x.len * f.lazy, f.lazy, x.len};
}
...
void solve() {
    ...
    while (q--) {
        ...
        if (type == 1) {
            int x;
            rd(x);
            ll val = seg.prod(0, x).mn;
            seg.apply(0, x, {val});
        } else if (type == 2) {
            int y;
            rd(y);
            ll val = seg.prod(y - 1, n).mn;
            seg.apply(y - 1, n, {val});
        } else {
            prt(seg.all_prod().sum);
        }
    }
}
