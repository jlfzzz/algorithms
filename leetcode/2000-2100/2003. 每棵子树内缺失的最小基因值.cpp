...
class Solution {
    static const int MAX_VAL = 100010;
    ...
    void pushup(int p) { tr[p].cnt = tr[tr[p].ls].cnt + tr[tr[p].rs].cnt; }
    ...
    void modify(int &p, int l, int r, int val) {
        ...
        int mid = (l + r) >> 1;
        if (val <= mid)
            modify(tr[p].ls, l, mid, val);
        else
            modify(tr[p].rs, mid + 1, r, val);
        pushup(p);
    }
    ...
    void dfs(int u, const vector<int> &nums) {
        modify(root[u], 1, MAX_VAL, nums[u]);
