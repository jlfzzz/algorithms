...
void solve() {
    ...
    auto getD = [&](int u, int v) -> ll { return lca.get_dis(u + 1, v + 1); };

    auto calc = [&](int u, int v) -> ll {
        if (u == v) {
            ll tot = (ll) n * (n + 1) / 2;
            ll bad = 0;
            ...
            for (int vv: adj[u]) {
                ll s;
                if (lca.depth[vv + 1] > lca.depth[u + 1]) {
                    s = sz[vv];
                } else {
                    s = n - sz[u];
                }
        ...
        int u1 = u + 1, v1 = v + 1;
        int p1 = lca.get_lca(u1, v1);

        if (p1 == u1 || p1 == v1) {
            int anc = (p1 == u1) ? u1 : v1;
            int des = (p1 == u1) ? v1 : u1;

            int dis = lca.depth[des] - lca.depth[anc] - 1;
            int child = lca.get_kth_ancestor(des, dis);

            return (ll) sz[des - 1] * (n - sz[child - 1]);
        } else {
            return (ll) sz[u] * sz[v];
        }
    };
