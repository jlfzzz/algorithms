...
S op(S a, S b) { return {max(a.mx1, b.mx1), min(a.mn1, b.mn1), max(a.mx2, b.mx2), min(a.mn2, b.mn2)}; }
...
S newnode(ll x, ll y) { return {x + y, x + y, x - y, x - y}; }
...
void solve() {
    ...
    vector<S> tree_arr(n);
    L(i, 0, n - 1) { tree_arr[i] = newnode(a[i].fi, a[i].se); }
    ...
    L(i, 1, q) {
        ...
        if (type == 1) {
            ...
            ll ans = 0;
            ans = max(ans, abs(res.mx1 - sum));
            ans = max(ans, abs(res.mn1 - sum));

            ans = max(ans, abs(res.mx2 - diff));
            ans = max(ans, abs(res.mn2 - diff));
