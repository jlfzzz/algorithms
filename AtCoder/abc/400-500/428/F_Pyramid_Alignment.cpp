...
pair<ll, ll> get_interval(S s, int i) {
    if (s.type == 0) {
        return {s.pos, s.pos + W[i]};
    } else {
        return {s.pos - W[i], s.pos};
    }
}
...
void solve() {
    ...
    vector<S> initial_vec(N + 1, {0, 0});

    atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg(initial_vec);
    ...
    while (Q--) {
        ...
        if (type == 1) {
            ...
            while (low <= high) {
                ...
                S state = seg.get(mid);
                auto [l, r] = get_interval(state, mid);
