...
int get_root(int x, int k) {
    if (k == 1)
        return x;
    int r = round(pow(x, 1.0 / k));
    auto check = [&](int val) {
        long long res = 1;
        for (int i = 0; i < k; i++) {
            res *= val;
...
void solve() {
    ...
    int max_len_limit = 20;
    vector<vector<int>> len_divs(max_len_limit + 1);
    for (int i = 1; i <= max_len_limit; i++) {
        for (int j = 1; j <= i; j++) {
            if (i % j == 0)
                len_divs[i].push_back(j);
        }
    }

    vector<vector<Z>> f(u + 1, vector<Z>(22, 0));
    vector<vector<Z>> g(u + 1, vector<Z>(22, 0));
    vector<int> mx(u + 1, 0);
    ...
    for (int i = 1; i <= u; i++) {
        for (int j = 2; i * j <= u; j++) {
            ...
            for (int l = 0; l <= mx[i]; l++) {
                ...
                f[next_prod][l + 1] += f[i][l] * cnt[j];
                mx[next_prod] = max(mx[next_prod], l + 1);
            }
        }
    }
    ...
    for (int i = 2; i <= u; i++) {
        ...
        for (int l = 1; l <= mx[i]; l++) {
            ...
            Z sum_periodic = 0;
            for (int d: len_divs[l]) {
                if (d == l)
                    continue;

                int repeat_times = l / d;
                int root = get_root(i, repeat_times);

                if (root != -1) {
                    sum_periodic += g[root][d] * d;
                }
            }

            g[i][l] = (f[i][l] - sum_periodic) / l;
            for (int d: len_divs[l]) {
                int repeat_times = l / d;
                int root = get_root(i, repeat_times);
                if (root != -1) {
