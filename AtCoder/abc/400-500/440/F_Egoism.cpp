...
void solve() {
    ...
    while (q--) {
        ...
        int oldv = arr[idx].fi;
        int oldst = arr[idx].se;

        sum -= oldv;
        upd(oldv, -1);

        if (oldst == 1)
            one.erase(one.find(oldv));
        else
            two.erase(two.find(oldv));
        ...
        int k = n - c2;
        int r = seg.max_right(1, [&](S x) { return x.cnt <= k; });

        S res = seg.prod(1, r);
        ll loss = res.sum + (ll) (k - res.cnt) * r;

        if (k > 0 && !one.empty() && !two.empty()) {
            int mx1 = *one.rbegin();
            int mn2 = *two.begin();
