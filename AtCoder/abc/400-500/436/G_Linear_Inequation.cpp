...
mint bostan_mori(vector<mint> P, vector<mint> Q, ll M) {
    while (M > 0) {
        int d = SZ(Q);
        ...
        auto U = atcoder::convolution(P, Q_neg);
        auto V = atcoder::convolution(Q, Q_neg);

        vector<mint> new_P;
        for (int i = (M % 2); i < SZ(U); i += 2) {
            new_P.pb(U[i]);
        }
        P = new_P;
...
void solve() {
    ...
    auto comp = [](const vector<mint> &a, const vector<mint> &b) { return a.size() > b.size(); };
    priority_queue<vector<mint>, vector<vector<mint>>, decltype(comp)> pq(comp);
    ...
    while (pq.size() > 1) {
        ...
        auto p2 = pq.top();
        pq.pop();
        pq.push(atcoder::convolution(p1, p2));
    }

    vector<mint> Q = pq.top();
    vector<mint> P = {1};

    prt(bostan_mori(P, Q, M).val());
}
