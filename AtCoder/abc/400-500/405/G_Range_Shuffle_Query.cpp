...
void solve() {
    ...
    int V = n;
    int BV = max(1, (int) sqrt(V) + 1);
    int block_cnt = (V + BV - 1) / BV;
    vector<int> freq(V + 2);
    vector<int> block_sum(block_cnt + 2);
    vector<Z> block_prod(block_cnt + 2, Z(1));

    auto apply = [&](int val, int delta) {
        int blk = (val - 1) / BV + 1;
        int oldCnt = freq[val];
        block_sum[blk] += delta;
        block_prod[blk] *= comb.fac(oldCnt);
        freq[val] += delta;
        block_prod[blk] *= comb.invfac(freq[val]);
    };

    auto add = [&](int x) { apply(x, 1); };

    auto del = [&](int x) { apply(x, -1); };

    auto get = [&](int bound) -> pair<int, Z> {
        int upto = bound - 1;
        if (upto <= 0) {
            return {0, Z(1)};
        }
        ...
        Z prod = Z(1);
        int start = 1;
        for (int blockIndex = 1; blockIndex <= block_cnt && start + BV - 1 <= upto; blockIndex++) {
            total += block_sum[blockIndex];
            prod *= block_prod[blockIndex];
            start += BV;
        }
        for (int i = start; i <= upto; i++) {
            total += freq[i];
            prod *= comb.invfac(freq[i]);
        }
        return {total, prod};
    };
    ...
    F(i, 1, q) {
        ...
        auto [tot, prod] = get(x);
        ans[id] = comb.fac(tot) * prod;
    }
    F(i, 1, q) { prt(ans[i].val()); }
}
