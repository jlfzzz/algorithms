...
void solve() {
    ...
    atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg(leaves);
    ...
    for (int i = 0; i < m; i++) {
        ...
        S range_val = seg.prod(l, r);

        ll total_stones = range_val.sum;
        ll length = range_val.len;
        ll avg = (total_stones * modInverse(length)) % MOD;
