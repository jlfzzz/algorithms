...
S op(S a, S b) { return {std::max(a.val, b.val)}; }

S e() { return {-1000000000000000000LL}; }

S mapping(F f, S x) { return {x.val + f.add}; }
...
void solve() {
    ...
    for (int j = 1; j < n; j++) {
        ...
        last[val] = j;
        if (j >= 2) {
            long long current_max = seg.prod(1, j).val;
            ans = max(ans, current_max + suf[j + 1]);
        }
    }
