...
S op(S a, S b) { return {a.sum + b.sum, min(a.mn, b.mn), max(a.mx, b.mx), a.len + b.len}; }
...
void solve() {
    ...
    while (q--) {
        ...
        ll target = g - idx; 
        ll cur = seg.get(idx).sum; 

        if (target == cur)
            continue;

        if (target > cur) {
            int r = seg.max_right(idx, [&](S x) { return x.mx < target; });

            S s = seg.prod(idx, r);
            total_ans += (target * s.len - s.sum);

            seg.apply(idx, r, {target});

        } else {
            int l = seg.min_left(idx + 1, [&](S x) { return x.mn > target; });

            S s = seg.prod(l, idx + 1);
            total_ans += (s.sum - target * s.len);

            seg.apply(l, idx + 1, {target});
        }
    }
