...
void solve() {
    ...
    int ts = 0;
    vl dis(n + 1), tin(n + 1), tout(n + 1), pa(n + 1);
    auto dfs = [&](auto &&dfs, int u, int fa, ll sum) -> void {
        pa[u] = fa;
        tin[u] = ++ts;
        dis[u] = sum;
        for (auto [v, w]: g[u]) {
    ...
    U(i, 1, n) {
        ...
        while (cur != 1 && !vis[cur]) {
            ...
            seg.apply(tin[cur], tout[cur] + 1, {w});
