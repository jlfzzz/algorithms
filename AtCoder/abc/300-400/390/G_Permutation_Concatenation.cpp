...
mint nCr(int n, int r) {
    if (r < 0 || r > n)
        return 0;
    return fact[n] * infact[r] * infact[n - r];
}
...
void solve2() {
    ...
    for (int len = 1; len <= 6; len++) {
        ...
        // NTT 卷积合并
        Q = atcoder::convolution(Q, P);
    }

    mint ans = 0;

    // 计算答案
    // 对于每种长度 L，我们需要计算 Q(x) / (1 + 10^L x) 的各项系数 b_k
    // 然后累加 b_k * k! * (N-1-k)! * sum_len[L]
    for (int len = 1; len <= 6; len++) {
        if (cnt[len] == 0)
            continue;
        ...
        // 线性递推做多项式除法
        // Q(x) = (1 + wx) * B(x)
        // Q_k = b_k + w * b_{k-1}  =>  b_k = Q_k - w * b_{k-1}

        // b_0 = Q_0 = 1
        mint cur_b = Q[0];

        // 累加系数因子： sum_{k=0}^{N-1} b_k * k! * (N-1-k)!
        mint factor = 0;

        // k = 0 的情况
        factor += cur_b * fact[0] * fact[n - 1];

        for (int k = 1; k < n; k++) {
            // 计算 b_k
            cur_b = Q[k] - w * cur_b;
            // 累加
            factor += cur_b * fact[k] * fact[n - 1 - k];
        }

        ans += sum_len[len] * factor;
    }
...
void solve() {
    ...
    F(target, 1, 6) {
        ...
        F(j, 1, 6) {
            ...
            // 构造这一组的二项式展开: (1 + 10^j * x)^count
            vector<mint> group_poly(count + 1);
            mint w = 1;
            mint w_base = p10[j];

            F(k, 0, count) {
                group_poly[k] = nCr(count, k) * w;
                w *= w_base;
            }

            // NTT 卷积合并
            poly = atcoder::convolution(poly, group_poly);
        }
        ...
        // 累加所有可能的后缀长度 k
        // poly[k] = 选k个数在后面的位权总和
        // 乘上 k! (后缀排列) 和 (n-1-k)! (前缀排列)
        for (int k = 0; k < max_k && k < n; k++) {
            group_factor += poly[k] * fact[k] * fact[n - 1 - k];
        }

        total_ans += sum_len[target] * group_factor;
    }
