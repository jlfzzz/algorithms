...
void solve() {
    ...
    for (int i = 0; i <= max_idx; ++i) {
        Z val_f = fact[i] / H[i + 2];
        f.push_back(val_f);

        Z val_g = q_pow * invFact[i];
        g.push_back(val_g);
    ...
    vector<Z> conv = atcoder::convolution(f, g);
    prt(H[n].val());
    ...
    for (int k = 2; k <= n; ++k) {
        ...
        Z ans = 0;
        if (idx >= 0 && idx < SZ(conv)) {
            ans = H[n] * p.pow(K + 1) * invFact[K] * conv[idx];
        }
        prt(ans.val());
    }
}
