        ...
        for (int j = 0; j < 2; j++) {
            for (int k = 0; k < 2; k++) {
                for (int p = 0; p < 2; p++) {
                    ...
                    if (a.v[i][k] > -INF && b.v[p][j] > -INF) {
                        res.v[i][j] = max(res.v[i][j], a.v[i][k] + b.v[p][j]);
                    }
                }
            }
        }
...
struct Lamp {
    ...
    bool operator<(const Lamp &other) const { return val < other.val; }
};
...
void solve() {
    ...
    vector<S> init_state(n + 1, make_inactive());
    atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg(init_state);
    ...
    for (int r = 0; r < n; r++) {
        int r_id = lamps[r].id;
        seg.set(r_id, make_active());

        while (true) {
            S root = seg.all_prod();
            int max_independent_set = max({root.v[0][0], root.v[0][1], root.v[1][0], root.v[1][1]});

            if (max_independent_set >= m) {

                ans = min(ans, lamps[r].val - lamps[l].val);

                int l_id = lamps[l].id;
                seg.set(l_id, make_inactive());
                l++;
            } else {
                break;
            }
