...
S op(S a, S b) { return {a.s + b.s, a.c + b.c}; }

S e() { return {0, 0}; }

S mapping(F f, S x) {
    double ns = x.s * f.c + x.c * f.s;
    double nc = x.c * f.c - x.s * f.s;
    return {ns, nc};
}

F composition(F f, F g) {
    double ns = f.s * g.c + f.c * g.s;
    double nc = f.c * g.c - f.s * g.s;
    return {ns, nc};
}
...
void solve() {
    ...
    vector<S> tree_arr(n);
    For(i, n) { tree_arr[i] = {sin(a[i]), cos(a[i])}; }
    ...
    while (m--) {
        ...
        if (op == 1) {
            ...
            seg.apply(l - 1, r, {sin(v), cos(v)});
        } else {
            int l, r;
            read(l, r);

            cout << seg.prod(l - 1, r).s << "\n";
        }
    }
}
