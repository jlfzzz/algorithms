...
void solve() {
    ...
    F(i, 0, n - 1) {
        ...
        F(j, 0, k - 1) {
            rd(a[i].vals[j]);
            mn = min(mn, a[i].vals[j]);
            mx = max(mx, a[i].vals[j]);
        }
        a[i].mn = mn;
        a[i].mx = mx;
        ranges::sort(a[i].vals);
    }

    multiset<int> vis;
    F(i, 0, n - 1) {
        auto pt = vis.lower_bound(a[i].vals[0]);
        int chosen = a[i].vals[0];

        for (size_t j = 1; j < a[i].vals.size(); j++) {
            auto pt1 = vis.lower_bound(a[i].vals[j]);
            if (pt != pt1)
                break;
            chosen = a[i].vals[j];
        }
    ...
    int sz0 = SZ(a[0].vals);
    vector<Z> pref(sz0);
    F(j, 0, sz0 - 1) { pref[j] = (j == 0 ? Z(0) : pref[j - 1]) + 1; }

    F(i, 1, n - 1) {
        auto &cur_vals = a[i].vals;
        auto &pre_vals = a[i - 1].vals;

        int sz = SZ(cur_vals);
        vector<Z> next_pref(sz);

        bool strict = a[i - 1].id > a[i].id;
        ...
        F(j, 0, sz - 1) {
            ...
            if (x > a[i].sort_key) {
                next_pref[j] = running_sum;
                continue;
            }

            int idx = -1;
            if (strict) {
                auto it = ranges::lower_bound(pre_vals, x);
                idx = int(it - pre_vals.begin()) - 1;
            } else {
                auto it = ranges::upper_bound(pre_vals, x);
                idx = int(it - pre_vals.begin()) - 1;
            }

            if (idx >= 0) {
                running_sum += pref[idx];
            }
