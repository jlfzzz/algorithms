...
void solve() {
    ...
    vector<vector<Z>> dp(n + 1, vector<Z>(sum + 1, 0));
    dp[0][0] = 1;

    for (int x: a) {
        D(i, n, 1) {
            D(s, sum, x) { dp[i][s] += dp[i - 1][s - x]; }
        }
    }
    ...
    for (int x: a) {
        ...
        F(i, 1, n) {
            F(s, x, sum) { g[i][s] -= g[i - 1][s - x]; }
        }

        F(k, 2, n) {
            ll lim = (ll) (k - 1) * x;

            for (int s = 0; s < lim && s <= sum; s++) {
                cnt[k] += g[k - 1][s];
            }
        }
    }
    ...
    vector<Z> ans;
    F(k, 2, n) {
        Z total_schemes = comb.C(n, k);
        Z res = cnt[k] / (total_schemes * k);
        ans.pb(res);
    }
