...
void solve() {
    ...
    vi preMax(n + 1), preVal(n + 1);
    {
        int mx = 0;
        int cnttt = 0;
        F(i, 1, n) {
            cnttt++;
            preMax[i] = max(preMax[i - 1], cnttt);
            if (a[i] > mx) {
                mx = a[i];
                cnttt = 0;
            }

            preVal[i] = max(preVal[i - 1], a[i]);
        }
    }
    ...
    F(_, 1, q) {
        ...
        if (type == 1) {
            ...
            int l2 = ranges::upper_bound(preMax, k) - preMax.begin();
            limit = min(limit, l2 - 1);

            int x = a[i];
            auto itR = ranges::upper_bound(bkt[x], limit);
            auto itL = ranges::lower_bound(bkt[x], i);

            int tot = itR - itL;

            bool gt = (a[i] > preVal[i - 1]);
            int exp = gt ? (tot - 1) : tot;

            Z ans = Z(1) / q_pow(2, exp);
